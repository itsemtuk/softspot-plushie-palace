...
-- Create secure profile view that only shows appropriate fields based on privacy
CREATE OR REPLACE VIEW public.profile_safe_view AS
SELECT 
  user_uuid,
  CASE 
    WHEN is_private = true AND user_uuid != (SELECT users.id FROM users WHERE users.clerk_id = (auth.jwt() ->> 'sub'::text))
    THEN NULL 
    ELSE bio 
  END as bio,
  CASE 
...
GRANT SELECT ON public.profile_safe_view TO anon, authenticated;
