...
SELECT DISTINCT
  ...
  CASE 
    WHEN c.participants[1] = auth.uid()::text THEN c.participants[2]
...
USING (
  ...
  AND (
    ...
    EXISTS (
      ...
      WHERE f.follower_id = auth.uid() 
      AND f.following_id::text = CASE 
        WHEN participants[1] = auth.uid()::text THEN participants[2]
        ELSE participants[1]
      END
    )
    OR
