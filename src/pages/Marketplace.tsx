      ...
      try {
        ...
        if (listings && listings.length > 0) {
          const formattedItems: MarketplacePlushie[] = listings.map(listing => ({
            ...
            title: (listing.title as string) || 'Untitled Item',
            name: (listing.title as string) || 'Untitled Item',
            price: (listing.price as number) || 0,
            image: listing.image_urls?.[0] || '',
            brand: (listing.brand as string) || 'Unknown',
            condition: (listing.condition as string) || 'used',
            description: (listing.description as string) || '',
            tags: listing.brand ? [(listing.brand as string).toLowerCase()] : [],
            likes: 0,
            comments: 0,
            forSale: true,
            userId: listing.user_id as string,
            username: (listing.users as any)?.username || (listing.users as any)?.first_name || 'User',
